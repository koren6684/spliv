...
to allow the possible direct effect to vary with an observed exposure.
The package supports both bounded UCI and local-to-zero (LTZ)
sensitivity.
...
Sensitivity paths report how the estimated interval changes over a
pre-specified range of direct-effect magnitudes. The tipping point is
the first value on the supplied grid at which the interval includes
zero.
